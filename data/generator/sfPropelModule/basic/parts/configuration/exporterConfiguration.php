...
<?php unset($this->config['exportation']['helperClass']) ?>
  ...
  {
    return '<?php echo isset($this->config['exportation']['ajaxIndicatorPath'])? $this->config['exportation']['ajaxIndicatorPath'] : '/pmPropelGeneratorPlugin/images/ajax-loader.gif' ?>';
  }
...
<?php unset($this->config['exportation']['helperUserClass']) ?>
  ...
  public function getExportationForm($defaults = array(), $options = array())
  {
    ...
    return new $formClass(array(), array_merge(array('fields' => $this->getExportationFieldSelectionDecorators(), 'title' => $this->getExportationFormTitle(), 'type' => $this->getExportationType(), 'allowUserTypeSelection' => $this->getExportationAllowUserTypeSelection()), $options));
  }
...
<?php unset($this->config['exportation']['allowUserTypeSelection']) ?>
  ...
  public function getExportationCustomRow()
  {
    return <?php echo $this->asPhp(isset($this->config['exportation']['customRow'])? $this->config['exportation']['customRow'] : array()) ?>;
  }
<?php unset($this->config['exportation']['customRow']) ?>
...
<?php unset($this->config['exportation']['fieldSelection']) ?>
  ...
  public function getExportationFieldSelectionDecorators()
  {
    ...
    {
      $fields[$id] = pmExporterDecoratorFactory::getInstance($f);
      $fields[$id]->setId($id);
    }
    return $fields;
  }
<?php unset($this->config['exportation']['fieldSelection']) ?>
  ...
  public function getExportationHeaders()
  {
    ...
    {
      $f = pmExporterDecoratorFactory::getInstance($f);
      $headers[] = $f->getLabel();
    }
    return $headers;
  }

  public function getExportationFormTitle()
  {
    return <?php echo $this->asPhp(isset($this->config['exportation']['form_title'])? $this->config['exportation']['form_title'] : 'Report') ?>;
  }
<?php unset($this->config['exportation']['form_title']) ?>

  public function getExportationTitle()
  {
    return <?php echo $this->asPhp(isset($this->config['exportation']['title'])? $this->config['exportation']['title'] : 'Report') ?>;
  }
<?php unset($this->config['exportation']['title']) ?>

  public function getExportationDefaultType()
  {
    return pmExporterTypes::EXPORT_TYPE_XLS;
  }
...
<?php unset($this->config['exportation']['type']) ?>
  ...
  public function getExportationFileExtension($type = null)
  {
    ...
    return pmExporterTypes::getFileExtension($type);
  }
  ...
  {
    $type = is_null($type)? $this->getExportationType() : $type;
    return pmExporterTypes::getMimeType($type);
  }
